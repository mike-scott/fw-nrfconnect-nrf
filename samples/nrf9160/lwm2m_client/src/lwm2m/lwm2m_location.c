...
#define LOG_MODULE_NAME app_lwm2m_loc
 ...
 */
static void double_to_float32_value(double val,
				    struct float32_value *f32_val)
{
	f32_val->val1 = (int)val;
	f32_val->val2 = (val - (int)val) * 1000000;
}
...
static void gps_trigger_handler(struct device *dev, struct gps_trigger *trigger)
{
	...
	if (k_uptime_get_32() - timestamp_prev <
	    K_SECONDS(CONFIG_APP_HOLD_TIME_GSP)) {
		return;
	}
